...
def get_city(city):
    ...
    cursor = conn.cursor()
    query = """SELECT city, lat, lng, country, iso2 \
               FROM simplemaps_worldcities_basic \
...
def get_info():
    ...
    query = """SELECT country_code, \
                      foundation_year, \
                      surface, language, currency, continent, capital
               FROM informations"""
    cursor.execute(query)
    results = cursor.fetchall()
    conn.close()
    data_frame = pd.DataFrame(results,
                              columns=["country_code", "foundation_year", "surface (km²)", "language", "currency",
                                       "continent", "capital"])
    return data_frame
